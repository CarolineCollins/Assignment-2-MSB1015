...
Nevertheless, this code, preparatory to building a predictive model from the descriptor matrix, is set up to remove
...
```
Check the distribution of the boiling points of the training dataset.
...
```{r}
plot(bp_model_1, ncomp = 1, asp = 1, line = TRUE)
```
...
##Model 2 A data-driven kitchen sink version
...
and create one big decriptor_matrix_2
```{r}
...
```
ased on the elbow plot , (see above) a model with 3 latent variables appears optimum with good performance without adding  too much complexity. What's the predictive power of this data-driven model with 3 latent variables?
...
It is interesting to note that, based on the earlier elbow plot depicting the results of the cross-validation RMSEP's from the training set alone,  the choice of 2, 3,4,5 or 6 latent variables for the model looked optimum, with the final model choice of 3 latent variables meeting the criterion 'As simple as possible, but not simpler than really needed', Whereas, now we can see here the error measurement at 10 latent variables (ncomp = 10) gives the lowest actual error on the independent randomly sampled test data.

###remember how the correlation between SMILES string length and boiling point was 0.93...
```{r}
cor(pred_bp_1,bp_test$boilingpoint)
```

Compare: what is now the correlation between predicted and observed boiling points? 0.99
```{r}
cor(pred_bp_2,bp_test_2$boilingpoint)
```
So the data-driven model with few latent variables created from a large number of variables with a chemical structural meaning is an improvement on the first simple model that I created from handpicked CDK descriptors with only one latent variable. That first model has a similar predict/oberve correlation to the ultra-simple SMILES stringlength vs boiling point comparison.

**The Final model RMSEP is 42.77 on the test set with 3 latent variables.**


Comparing this to literature : Wiener constructed a relationship with 3 variables n (number of Carbon atoms) and w and p.

Repeat and apply to **more chemical structures** possibly alcohols.
Explain the differences in results. Currently there is a lack of available boiling point data in WikiData on alcohols and other organic molecules.
