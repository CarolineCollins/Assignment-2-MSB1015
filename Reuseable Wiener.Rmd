...
```{r}
sparql_query_missingbp <- 'SELECT DISTINCT ?alkanes ?alkanesLabel ?SMILES ?boilingpoint
WHERE {
    ?alkanes wdt:P31/wdt:P279* wd:Q41581 .
    ?alkanes wdt:P233 ?SMILES .
    MINUS {?alkanes wdt:P2102 ?boilingpoint .}
    SERVICE wikibase:label { bd:serviceParam wikibase:language "en"}
}'
results_missingbp = query_wikidata(sparql_query_missingbp)
```
```{r}
sparql_query <- 'SELECT DISTINCT ?alkanes ?alkanesLabel ?boilingpoint 
WHERE {
    ?alkanes wdt:P31/wdt:P279* wd:Q41581 .
    
    ?alkanes wdt:P2102 ?boilingpoint .
    SERVICE wikibase:label { bd:serviceParam wikibase:language "en"}
}'
